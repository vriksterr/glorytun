...
int
gt_argz_ifname(int argc, char **argv, void *data)
{
    char *ifname = data;
    ...
    if (argz_help_me(argc, argv)) {
        ...
        if (!len || len >= IFNAMSIZ) {
            gt_log("Option %s is not a valid interface name\n", argv[0]);
            return -1;
        }
        memcpy(ifname, argv[1], len + 1);
        return argc - 2;
    } else {
        gt_log("Option %s requires an interface name\n", argv[0]);
    }
    return -1;
}
